...
from referee.game import PlaceAction, Coord, BOARD_N
...
def precompute_moves_for_coord(r, c):
    ...
    for template in all_templates:
        ...
        if (template[0]).v1==Vector2(-1,0):
            direction = 'up'
        elif (template[0]).v1==Vector2(0,1):
            direction = 'right'
        elif (template[0]).v1==Vector2(1,0):
            direction = 'down'
        elif (template[0]).v1==Vector2(0,-1):
            direction = 'left'
        ...
        for shape in template:
            ...
            # Obtain current shape by summing my coord and the action
            ...
            moves[direction].append(current_shape)

    return moves

# Add it all to one big dictionary of dictionaries
precomputed_moves = {}
for r in range(BOARD_N):
    for c in range(BOARD_N):
        precomputed_moves[Coord(r, c)] = precompute_moves_for_coord(r, c)
